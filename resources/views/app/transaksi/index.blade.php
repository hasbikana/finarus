...
<button onclick="openModal()" class="w-full sm:w-auto h-9 text-sm bg-primary text-primary-foreground hover:bg-primary/90 transition-all duration-300 px-4 rounded-md font-medium flex items-center justify-center gap-2">
    + Tambah Transaksi
</button>
@endsection
...
<div class="bg-card rounded-lg shadow-lg p-5">
    <form method="GET" action="{{ route('transaksi') }}" class="flex flex-col md:flex-row gap-4 mb-5">
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('transaksi') }}" class="h-9 px-4 rounded-md border border-border hover:bg-muted transition-colors text-sm font-medium flex items-center">Reset</a>
            @endif
        </div>
    </form>
        ...
        <table class="w-full">
            <thead class="border-b border-border">
                <tr>
                    ...
                    <th class="text-left py-3 px-4 font-semibold text-sm">Tipe</th>
            ...
            <tbody>
                ...
                <tr class="border-b border-border hover:bg-muted transition-colors">
                    ...
                    <td class="py-3 px-4 text-center">
                        <button onclick="editTxn({{ $txn->id }},'{{ addslashes($txn->description??'') }}',{{ $txn->amount }},'{{ $txn->type }}','{{ $txn->transaction_date->format('Y-m-d') }}',{{ $txn->category_id??'null' }},{{ $txn->account_id??'null' }})" class="text-primary hover:text-primary/80 text-xs font-medium">Edit</button>
                        <span class="mx-1 text-muted-foreground">|</span>
                        <form method="POST" action="{{ route('transaksi.destroy', $txn) }}" class="inline" onsubmit="return confirm('Hapus transaksi ini?')">
                            @csrf @method('DELETE')
...
<dialog id="modal-txn" class="p-0 rounded-lg shadow-xl backdrop:bg-black/50 max-w-md w-full bg-card text-foreground">
    <form id="form-txn" method="POST" action="{{ route('transaksi.store') }}" class="p-6" x-data>
        @csrf
        ...
        <div class="space-y-4">
            ...
            <div><label class="block text-sm font-medium mb-1">Deskripsi</label><input type="text" name="description" id="txn-desc" required placeholder="Contoh: Belanja Bulanan" class="w-full h-9 px-3 rounded-md border border-border bg-background text-foreground"></div>
            <div><label class="block text-sm font-medium mb-1">Kategori</label><select name="category_id" id="txn-cat" required class="w-full h-9 px-3 rounded-md border border-border bg-card text-foreground"><option value="">Pilih</option>@foreach($categories as $cat)<option value="{{ $cat->id }}">{{ $cat->icon }} {{ $cat->name }}</option>@endforeach</select></div>
            <div><label class="block text-sm font-medium mb-1">Akun</label><select name="account_id" id="txn-acc" class="w-full h-9 px-3 rounded-md border border-border bg-card text-foreground"><option value="">Pilih</option>@foreach($accounts as $acc)<option value="{{ $acc->id }}">{{ $acc->name }}</option>@endforeach</select></div>
            <div><label class="block text-sm font-medium mb-1">Tipe</label><select name="type" id="txn-type" required class="w-full h-9 px-3 rounded-md border border-border bg-card text-foreground"><option value="expense">Pengeluaran</option><option value="income">Pemasukan</option></select></div>
            <div><label class="block text-sm font-medium mb-1">Jumlah (Rp)</label><input type="number" name="amount" id="txn-amount" required min="1" step="1" placeholder="50000" class="w-full h-9 px-3 rounded-md border border-border bg-background text-foreground"></div>
            <div><label class="block text-sm font-medium mb-1">Tanggal</label><input type="date" name="transaction_date" id="txn-date" required class="w-full h-9 px-3 rounded-md border border-border bg-background text-foreground"></div>
...
<script>
function openModal() {
    var f = document.getElementById('form-txn');
    f.action = '{{ route("transaksi.store") }}';
...
function editTxn(id, desc, amount, type, date, catId, accId) {
    var f = document.getElementById('form-txn');
    f.action = '{{ url("transaksi") }}/' + id;
    document.getElementById('txn-method').value = 'PUT';
    document.getElementById('txn-desc').value = desc;
    document.getElementById('txn-cat').value = catId || '';
    document.getElementById('txn-acc').value = accId || '';
