    ...
    <div class="bg-card rounded-lg shadow-lg p-5 hover:shadow-2xl hover:scale-105 transition-all duration-500 ease-out">
        <div class="flex items-center justify-between flex-wrap gap-3">
            <div class="flex items-center gap-3">
                ...
                <div><h3 class="font-semibold text-foreground">{{ $account->name }}</h3><p class="text-xs text-muted-foreground">{{ $account->type==='cash'?'Cash':($account->type==='ewallet'?'E-Wallet':($account->type==='credit_card'?'Kartu Kredit':'Bank')) }} &bull; {{ $account->provider }}@if($account->email_scopes && count($account->email_scopes) > 0)<span class="ml-2 px-1.5 py-0.5 rounded text-[10px] bg-primary/10 text-primary">{{ count($account->email_scopes) }} email scope</span>@endif</p></div>
            </div>
            <div class="text-right"><p class="text-lg font-semibold text-foreground">Rp {{ number_format($account->balance,0,',','.') }}</p>
                <div class="flex gap-2 mt-2 justify-end">
                    <button onclick="editAcc(this)" data-id="{{ $account->id }}" data-name="{{ $account->name }}" data-provider="{{ $account->provider }}" data-type="{{ $account->type }}" data-num="{{ $account->account_number }}" data-balance="{{ $account->balance }}" data-logo="{{ $account->logo }}" data-scopes='{{ json_encode($account->email_scopes ?? []) }}' class="text-primary hover:text-primary/80 text-xs font-medium">Edit</button>
    ...
    <form id="form-acc" method="POST" action="{{ route('dompet.store') }}" class="p-6" onsubmit="var b=this.querySelector('button[type=submit]');b.disabled=true;b.innerHTML='⟳ Menyimpan...';b.classList.add('opacity-70')">
        ...
        <div class="space-y-4">
            ...
            <div id="scope-section">
                ...
                <div id="scope-tags" class="flex flex-wrap gap-1.5 mb-2"></div>
                <div id="scope-hidden"></div>
                <p id="scope-empty" class="text-xs text-muted-foreground">Belum ada scope. Tambahkan alamat email pengirim yang ingin difokuskan untuk akun ini.</p>
                <button type="button" onclick="fillDefaultScopes()" class="text-xs text-primary hover:underline mt-1">Isi email default provider</button>
                <x-input-error :messages="$errors->get('email_scopes')" class="text-sm mt-1" />
            </div>
...
function openModal(){clearScopes();document.getElementById('form-acc').action='{{ route("dompet.store") }}';document.getElementById('acc-method').value='POST';document.getElementById('acc-name').value='';document.getElementById('acc-provider').value='';document.getElementById('acc-type').value='bank';document.getElementById('acc-num').value='';document.getElementById('acc-balance').value='0';document.getElementById('acc-logo').value='';document.getElementById('acc-logo-display').value='';document.getElementById('acc-title').textContent='Tambah Akun';document.querySelectorAll('.logo-btn').forEach(b=>b.classList.remove('border-primary','ring-2','ring-primary/20'));document.getElementById('modal-acc').showModal()}
function editAcc(btn){var d=btn.dataset;document.getElementById('form-acc').action='{{ url("dompet") }}/'+d.id;document.getElementById('acc-method').value='PUT';document.getElementById('acc-name').value=d.name;document.getElementById('acc-provider').value=d.provider;document.getElementById('acc-type').value=d.type;document.getElementById('acc-num').value=d.num;document.getElementById('acc-balance').value=d.balance;document.getElementById('acc-logo').value=d.logo;document.getElementById('acc-logo-display').value=d.logo;document.getElementById('acc-title').textContent='Edit Akun';document.querySelectorAll('.logo-btn').forEach(b=>b.classList.remove('border-primary','ring-2','ring-primary/20'));var m=Array.from(document.querySelectorAll('.logo-btn')).find(b=>b.querySelector('img')?.alt===d.logo);if(m)m.classList.add('border-primary','ring-2','ring-primary/20');clearScopes();if(d.scopes)JSON.parse(d.scopes).forEach(function(v){scopeList.push(v)});renderScopes();document.getElementById('modal-acc').showModal()}
@if($errors->any())document.addEventListener('DOMContentLoaded',function(){@if(old('_method')==='PUT' && request()->route('account'))document.getElementById('form-acc').action='{{ url("dompet") }}/{{ request()->route("account")->id }}';document.getElementById('acc-method').value='PUT';document.getElementById('acc-title').textContent='Edit Akun';@endif @if(old('email_scopes'))JSON.parse('{{ json_encode(old("email_scopes"))|e("js") }}').forEach(function(v){scopeList.push(v)});renderScopes();@endif document.getElementById('modal-acc').showModal()});@endif
</script>
@endpush
